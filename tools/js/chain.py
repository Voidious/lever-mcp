...
def _find_first_non_operation_key(iterable):
    for k in iterable:
        if k != "operation":
            return k
    return None
...
def _check_and_set_param(
    param_name: str, arguments: dict, step_index: int, value
) -> dict | None:
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            result = _check_and_set_param(
                primary_param, arguments, i, unwrap_result(value)
            )
            ...
            result = _check_and_set_param(
                only_param, arguments, i, unwrap_result(value)
            )
            if result is not None:
                return result

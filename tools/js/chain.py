...
def _validate_and_assign_chained_param(
    step_index: int,
    param_name: str,
    arguments: dict,
    value: Any,
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            error = _validate_and_assign_chained_param(
            ...
            error = _validate_and_assign_chained_param(i, only_param, arguments, value)
            if error:
                return error

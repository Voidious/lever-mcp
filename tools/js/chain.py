...
def _validate_and_set_param(param_name, arguments, step_index, value):
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            result = _validate_and_set_param(primary_param, arguments, i, value)
            if result is not None:
                return result
        elif len(param_schema) == 1:
            only_param = next(iter(param_schema))
            result = _validate_and_set_param(only_param, arguments, i, value)
            if result is not None:
                return result

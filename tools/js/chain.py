...
def _find_non_operation_param(source):
    for k in source:
        if k != "operation":
            return k
    return None
...
def _validate_and_set_chain_param(
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            error = _validate_and_set_chain_param(arguments, primary_param, value, i)
            if error:
                return error
        elif len(param_schema) == 1:
            only_param = next(iter(param_schema))
            error = _validate_and_set_chain_param(arguments, only_param, value, i)
            if error:
                return error

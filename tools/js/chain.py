...
def _find_non_operation_key(iterable):
    for k in iterable:
        if k != "operation":
            return k
    return None
...
def _check_and_set_primary_param(param_name, arguments, step_idx, value, unwrap_result):
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            result = _check_and_set_primary_param(
                primary_param, arguments, i, value, unwrap_result
            )
            ...
            result = _check_and_set_primary_param(
                only_param, arguments, i, value, unwrap_result
            )
            if result is not None:
                return result

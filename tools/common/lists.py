...
def _normalize_int_param(value, default=1):
...
def _add_if_unique(item, seen, result):
    try:
        if item not in seen:
            seen.add(item)
            result.append(item)
    ...
    try:
        ...
        for sublist in items:
            if isinstance(sublist, list):
                for item in sublist:
                    _add_if_unique(item, seen, result)
            else:
                # Handle case where items is not a list of lists
                _add_if_unique(sublist, seen, result)
        return {"value": result}
...
def _add_unique_item(seen: set, result: list, k_hash, item):
    ...
    try:
        ...
        for index, item in enumerate(items, 1):
            ...
            try:
                ...
                _add_unique_item(seen, result, k_hash, item)
...
def _build_others_keys(expr_handler, expr, others):
    others_keys = set()
...
def op_difference_by(
    ...
    **kwargs,
    ...
    try:
        ...
        others_keys = _build_others_keys(expr_handler, diff_expr, others)
...
def op_intersection_by(
    ...
    **kwargs,
    ...
    try:
        # Extract keys from others list using expression
        others_keys = _build_others_keys(expr_handler, inter_expr, others)

...
def _normalize_int_param(param: Any, default: int = 1) -> int:
...
def _zip_transpose(items: list) -> dict:
    if not items:
        return {"value": []}
...
def _add_unique(element: Any, seen: set, result: list) -> None:
    try:
    ...
    try:
        ...
        for sublist in items:
            if isinstance(sublist, list):
                for item in sublist:
                    _add_unique(item, seen, result)
            else:
                # Handle case where items is not a list of lists
                _add_unique(sublist, seen, result)
        return {"value": result}
...
def _collect_others_keys(expr_handler, expression, others):
    others_keys = set()
    for index, item in enumerate(others, 1):
        key_val = expr_handler(expression, item, index, others)

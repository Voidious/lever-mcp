...
def _normalize_count(param: Any, default: int = 1) -> int:
...
def _zip_to_list_result(items: list) -> dict:
    if not items:
        return {"value": []}
...
def op_zip_lists(items: list, **kwargs) -> dict:
    ...
    return _zip_to_list_result(items)
...
def _add_unique_item(seen: set, result: list, value: Any) -> None:
    try:
    ...
    try:
        ...
        for sublist in items:
            if isinstance(sublist, list):
                for item in sublist:
                    _add_unique_item(seen, result, item)
            else:
                # Handle case where items is not a list of lists
                _add_unique_item(seen, result, sublist)
        return {"value": result}

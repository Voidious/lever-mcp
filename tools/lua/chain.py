...
def _validate_and_set_primary_param(i, param_name, arguments, value, unwrap_result):
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            result = _validate_and_set_primary_param(
                i, primary_param, arguments, value, unwrap_result
            )
            ...
            result = _validate_and_set_primary_param(
                i, only_param, arguments, value, unwrap_result
            )
            if result is not None:
                return result

...
def _check_and_set_param(
    param_name: str, step_index: int, arguments: dict, value: Any, unwrap_result_func
) -> dict | None:
    ...
    arguments[param_name] = unwrap_result_func(value)
    return None
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if tool_name == "generate":
            ...
            error_result = _check_and_set_param(
                primary_param, i, arguments, value, unwrap_result
            )
            ...
            error_result = _check_and_set_param(
                only_param, i, arguments, value, unwrap_result
            )
            if error_result is not None:
                return error_result

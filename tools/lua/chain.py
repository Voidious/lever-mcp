"""
Chain tool implementation for Lever MCP.

This module contains the chain tool functionality for chaining multiple
tool calls, piping the output of one as the input to the next.
"""
...
def _find_first_non_operation_key(keys):
    for k in keys:
        if k != "operation":
            return k
    return None
...
def _validate_and_set_param(arguments, param_name, value, step_index):
    if param_name in arguments:
        return {
            "value": None,
            "error": (
                f"Step {step_index}: Chaining does not allow specifying the "
                f"primary parameter '{param_name}' in params. The output "
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if not primary_param:
            primary_param = _find_first_non_operation_key(required)

        # Fallback to first non-'operation' parameter in schema
        if not primary_param and param_schema:
            primary_param = _find_first_non_operation_key(param_schema)
        ...
        if tool_name == "generate":
            ...
            result = _validate_and_set_param(arguments, primary_param, value, i)
            if result:
                return result
        elif len(param_schema) == 1:
            only_param = next(iter(param_schema))
            result = _validate_and_set_param(arguments, only_param, value, i)
            if result:
                return result

...
def _find_primary_param(iterable):
...
def _check_and_assign_param(param_name, arguments, step_index):
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if not primary_param:
            primary_param = _find_primary_param(required)

        # Fallback to first non-'operation' parameter in schema
        if not primary_param and param_schema:
            primary_param = _find_primary_param(param_schema)
        ...
        if tool_name == "generate":
            ...
            error = _check_and_assign_param(primary_param, arguments, i)
            if error:
                return error
            ...
            error = _check_and_assign_param(only_param, arguments, i)
            if error:
                return error

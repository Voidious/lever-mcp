...
def _find_non_operation_param(keys):
    for k in keys:
        if k != "operation":
            return k
    return None
...
def _set_chained_param(param_name: str, arguments: dict, value, step_index: int):
...
async def chain_tool(input: Any, tool_calls: List[Dict[str, Any]], mcp) -> dict:
    ...
    for i, step in enumerate(tool_calls):
        ...
        if not primary_param:
            primary_param = _find_non_operation_param(required)

        # Fallback to first non-'operation' parameter in schema
        if not primary_param and param_schema:
            primary_param = _find_non_operation_param(param_schema)
        ...
        if tool_name == "generate":
            ...
            error = _set_chained_param(primary_param, arguments, value, i)
            if error:
                return error
        elif len(param_schema) == 1:
            only_param = next(iter(param_schema))
            error = _set_chained_param(only_param, arguments, value, i)
            if error:
                return error

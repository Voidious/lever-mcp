...
def _convert_lua_dict_to_list(data: Any, null_sentinel: Any, convert_fn) -> list:
    keys = list(data.keys())
    if not keys:
        return []
        ...
        max_k = max(keys)
        return [
            convert_fn(data[k] if k in keys else None, null_sentinel)
            for k in range(1, max_k + 1)
        ]
    else:
        # Non-consecutive keys, convert to list of values
        return [convert_fn(v, null_sentinel) for v in data.values()]
        ...
        if "__type" in obj and "data" in obj:
            ...
            if obj_type == "list":
                ...
                if hasattr(data, "keys"):
                    ...
                    converted_data = _convert_lua_dict_to_list(

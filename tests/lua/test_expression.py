...
async def _call_lists_tool_and_assert(client, items, operation, expression, expected):
    result, error = await make_tool_call(
        client,
        "lists",
...
async def test_min_max_by_expression(
    ...
    result = await _call_lists_tool_and_assert(
...
async def assert_any_eval_succeeds(client, value, expression, expected_result):
    result, error = await make_tool_call(
...
async def test_any_eval_expression(client, value, expression, expected_result):
    await assert_any_eval_succeeds(client, value, expression, expected_result)

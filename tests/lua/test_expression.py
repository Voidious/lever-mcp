...
async def test_remove_by_expression(client, items, expression, expected_count):
    ...
    assert_no_error_with_length(error, result, expected_count)
...
async def assert_tool_call_result(client, items, operation, expression, expected):
    result, error = await make_tool_call(
        client,
        "lists",
...
async def test_min_max_by_expression(
    ...
    result = await assert_tool_call_result(

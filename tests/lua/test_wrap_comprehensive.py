...
class TestDictsWrapBehavior:
    ...
    @staticmethod
    def _evaluate_and_assert_wrapped(test_cases, module_name):
        for operation, expression, expected_type in test_cases:
            result = evaluate_expression(expression, {})
            assert isinstance(
                result, dict
            ), f"{module_name}.{operation} should return wrapped structure"
            assert (
                result.get("__type") == expected_type
            ), f"{module_name}.{operation} should wrap as {expected_type}"
            assert (
                "data" in result
            ), f"{module_name}.{operation} should have data field when wrapped"
    ...
    def test_dict_returning_operations_with_wrap_true(self):
        ...
        TestDictsWrapBehavior._evaluate_and_assert_wrapped(test_cases, "dicts")

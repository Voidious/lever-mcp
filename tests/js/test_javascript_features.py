...
async def _assert_eval_expression(client, expression, expected_result):
...
async def test_arrow_functions(client, expression, expected_result):
    ...
    await _assert_eval_expression(client, expression, expected_result)


# --- Template Literal Tests ---


async def _assert_expression_evaluates_to(client, context, expression, expected_result):
    result, error = await make_tool_call(
        client,
        "any",
        {"value": context, "operation": "eval", "expression": expression},
    )
    assert error is None
    assert result == expected_result
...
async def test_template_literals(client, expression, context_value, expected_result):
    ...
    await _assert_expression_evaluates_to(
        client, context_value, expression, expected_result
    )
...
async def test_truthy_falsy_behavior(client, value, expression, expected_result):
    ...
    await _assert_expression_evaluates_to(client, value, expression, expected_result)

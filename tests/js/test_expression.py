...
async def _call_lists_operation(client, items, operation, expression, expected):
    result, error = await make_tool_call(
        client,
        "lists",
...
async def test_min_max_by_expression(
    ...
    await _call_lists_operation(client, items, operation, expression, expected_value)


# --- Difference/Intersection By Expression Tests ---


def assert_success_with_count(error, result, expected_count):
    assert error is None, f"Expected no error but got: {error}"
    assert result is not None, "Expected result to be non-None"
    assert (
        len(result) == expected_count
    ), f"Expected {expected_count} items but got {len(result)}: {result}"
...
async def test_difference_intersection_by_expression(
    ...
    assert_success_with_count(error, result, expected_count)
...
async def _call_any_eval(client, value, expression, expected_result):
    result, error = await make_tool_call(
...
async def test_null_handling_expression(client, value, expression, expected_result):
    await _call_any_eval(client, value, expression, expected_result)
...
async def test_new_list_by_operations_expressions(
    ...
    await _call_lists_operation(client, items, operation, expression, expected_result)

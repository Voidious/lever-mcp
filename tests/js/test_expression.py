...
def _assert_result_count(error, result, expected_count):
    assert error is None
    assert result is not None
    assert len(result) == expected_count
...
async def test_difference_intersection_by_expression(
    ...
    _assert_result_count(error, result, expected_count)
...
async def assert_eval_tool_call(client, value, expression, expected_result):
    result, error = await make_tool_call(
...
async def test_null_handling_expression(client, value, expression, expected_result):
    await assert_eval_tool_call(client, value, expression, expected_result)

...
def assert_list_result_count(error, result, expected_count, message=""):
    msg_prefix = f"{message}: " if message else ""
    assert error is None, f"{msg_prefix}Expected no error, got: {error}"
    assert result is not None, f"{msg_prefix}Expected result to not be None"
    assert (
        len(result) == expected_count
    ), f"{msg_prefix}Expected {expected_count} items, got {len(result)}"
